...
def encodeText(plainText, offset):
	hexEncoded = plainText.decode()  # plainText.encode("hex")
...
def createChallenge():
	createKey("0101010101010101", "key1")  # createKey("INSERT_SOME_KEY","key1")
	createKey("FFFFFFFFFFFFFFFF", "key2")  # createKey("SOME_OTHER_KEY","key2")

	plainText = open('test.txt').read()
	key1 = open('key1', 'rb').read()

	byte = desEncrypt(plainText,key1)
	key2 = open('key2', 'rb').read()
